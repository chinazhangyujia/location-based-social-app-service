...
const findWSInRoom = (thread) => {
    return chatRooms[thread];
}
    ...
    wss.on('connection', async (ws, req) => {
        ...
        ws.on('message', async (message) => {
            ...
            try {
                ...
                if (event === 'message') {
                    const thread = messageObject.thread;
                    const sendTo = messageObject.sendTo;
                    const sendFrom = messageObject.sendFrom;
                    const content = messageObject.content;

                    const isMessageValid = await validateChatMessage(thread, sendTo, sendFrom, content);
                    if (!isMessageValid) {
                        console.log('message data is not valid');
                        return;
                    }

                    const storedMessage = await storeChatMessage(thread, sendTo, sendFrom, content);
                    const storedMessageObject = storedMessage.toObject();
                    storedMessageObject.type = 'message';

                    redisPublisher.publish(thread.toString(), JSON.stringify(storedMessageObject));
