...
router.get('/friendStatus', auth, async (req, res) => {
  try {
    ...
    const pendingRequest = await AddFriendRequest.findOne({
      $or: [
...
router.post('/handleFriendRequest', auth, async (req, res) => {
  try {
    ...
    const friendRequest = await AddFriendRequest.findByIdAndUpdate({ _id: requestId },
      { status: req.body.status },
      { session, new: true }).exec();
    ...
    if (friendRequest.status === 'accepted') {
      await Friend.findOneAndUpdate({ user: req.user._id, friendUser: friendRequest.fromUser }, { status: 'active' }, { upsert: true, session }).exec();
      await Friend.findOneAndUpdate({ user: friendRequest.fromUser, friendUser: req.user._id }, { status: 'active' }, { upsert: true, session }).exec();
    }
